...
function __powerline_carreira_ruby_prompt {
  local ruby_version=""
...
function __powerline_carreira_python_venv_prompt {
  local python_venv=""
...
function __powerline_carreira_scm_prompt {
  local color=""
  local scm_prompt=""
...
function __powerline_carreira_battery_prompt {
  local color=""
...
function __powerline_carreira_in_vim_prompt {
  if [ -n "$VIMRUNTIME" ]; then
    echo "${IN_VIM_THEME_PROMPT_TEXT}|${IN_VIM_THEME_PROMPT_COLOR}"
  fi
}

function __powerline_carreira_left_segment {
...
function __powerline_carreira_last_status_prompt {
...
function __powerline_carreira_prompt_command {
  ...
  for segment in $POWERLINE_PROMPT; do
    local info="$(__powerline_carreira_${segment}_prompt)"
    [[ -n "${info}" ]] && __powerline_carreira_left_segment "${info}"
  done
  [[ "${last_status}" -ne 0 ]] && __powerline_carreira_left_segment $(__powerline_carreira_last_status_prompt ${last_status})
